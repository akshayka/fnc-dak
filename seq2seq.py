...
class Seq2SeqModel(FNCModel):
	def add_hidden_op(self, scope, old_h=None):
		...
		inputs = self.add_embedding(scope, scope)
		seqlen = sequence_length(inputs)

		cell = tf.contrib.rnn.BasicRNNCell(num_units=self.config.hidden_sizes)
	    # cell = tf.contrib.rnn.AttentionCellWrapper(cell=cell,
	        # attn_length=????)

		if scope == 'bodies':
			inputs = tf.reverse_sequence(inputs, seqlen)

		outputs, h_bodies = tf.nn.dynamic_rnn(cell=cell, inputs=inputs,
	        sequence_length=body_seqlen, dtype=tf.float32, scope=scope, 
	        initial_state=old_h)

		return h

	def add_prediction_op(self):
		h_body = self.add_hidden_op('bodies')
		h = self.add_hidden_op('headlines', h_body)

		xav = tf.contrib.layers.xavier_initializer()

		U_pred = tf.get_variable("U_pred", (self.config.hidden_sizes, 
			self.config_n_classes), initializer=xav)
		b_pred = tf.get_variable("b_pred", (self.config.n_classes), 
			initializer=xav)
		pred = tf.matmul(h, U_pred) + b_pred
		return pred
...
def do_train(train_bodies, train_stances, dimension, embedding_path, config, 
	...
	with tf.Graph().as_default():
		...
		start = time.time()
		model = Seq2SeqModel(config, max_headline_len, max_body_len, embeddings,
...
if __name__ == "__main__":
	parser = argparse.ArgumentParser(description="Trains and tests Seq2SeqModel",
